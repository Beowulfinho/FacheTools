// Supabase Edge Function: send-household-message-planes
//
// Planes' counterpart to send-household-message (Financas). Called directly from the Planes
// client ("Avisar a família" button on a trip/aventura) — not by a database trigger. The
// caller's own Supabase session authenticates the request (verify_jwt is on), so this only has
// to figure out who else shares that caller's household and push a free-text message to their
// Planes-app device(s) via Firebase Cloud Messaging, on Planes' own channel/icon
// ("planes_alerts", see ensureNotificationChannel in Planes/index.html).
//
